...
@section('body')
    ...
    <div class="x-body">
        ...
        <table class="layui-table">
            ...
            <tr>
                <td>退款状态：</td>
                <td colspan="2">{{ $order->refundOrStatus }}，理由：{{ $order->refund_reason }}</td>
                <td>
                    <!-- 如果订单退款状态是已申请，则展示处理按钮 -->
                    @if($order->refund_status == \App\Models\Order::REFUND_STATUS_PROCESSING)
                        <button type="button" id="agreen" data-id="{{ $order->id }}" class="layui-btn" >同意</button>
                        <button type="button" id="disagreen" data-id="{{ $order->id }}" class="layui-btn" >不同意</button>
                    @endif
                </td>
            </tr>
...
@section('script')
    ...
    <script>
        ...
        $('#agreen').on('click', function () {
            ...
            $.ajax({
                url: '{{ route('admin.order.refund') }}',
                method: 'post',
                data: {
                    agreen: 'y',
                    oid: $oid
                },
                success: function (msg,res,data) {
                    if( data.status == 200 ) {
                       alert('操作成功');
                       setTimeout(function () {
